...
public class RedisOpsUtils {
    ...
    public static void hashSet(String key, String field, Object value) {
        redisTemplate.opsForHash().put(key, field, value);//向 Redis 数据库存储一个键值对的标准方法
    }
    ...
    public static boolean expireKey(String key, long timeout) {
        return redisTemplate.expire(key, timeout, TimeUnit.SECONDS);//为 Redis 中已有的某个键（Key）设置生存时间（TTL）的标准方法
    }
    ...
    /**
     * ZREVRANGE — 按分数从高到低取排名（带分数）
     * @param key ZSet key
     * @param start 起始排名（0 开始）
     * @param end 结束排名
     * @return Set<ZSetOperations.TypedTuple<Object>>
     */
    public static java.util.Set<ZSetOperations.TypedTuple<Object>> zRevRangeWithScores(String key, long start, long end) {
